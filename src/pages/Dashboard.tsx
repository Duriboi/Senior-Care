    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <section>
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs text-slate-500 bg-slate-100 px-3 py-1 rounded-full">실시간 업데이트 중</span>
        </div>
          ...
          {redFlags.map((flag, i) => (
            <motion.div 
              ...
              className={cn(
                ...
                flag.color === 'error' ? "border-error" : "border-tertiary"
              )}
            >
              <div className={cn(
                "p-2 rounded-lg",
                flag.color === 'error' ? "bg-error-container text-error" : "bg-tertiary/10 text-tertiary"
              )}>
                <flag.icon size={20} />
              </div>
              <div>
                <p className={cn(
                  ...
                  flag.color === 'error' ? "text-error" : "text-tertiary"
                )}>{flag.type}</p>
                <p className="text-sm font-medium text-slate-900">{flag.resident}</p>
                <p className="text-lg font-black mt-1">{flag.value}</p>
                <p className="text-[10px] text-slate-500 mt-1">{flag.time}</p>
              </div>
            </motion.div>
          ))}
        </div>
                  ...
                  <div className="min-w-[140px]">
                    ...
                    <div className="mt-1">
                      <span className={cn(
                        ...
                        item.risk === 'High' ? "bg-error-container text-error" : "bg-tertiary/10 text-tertiary"
                      )}>
                        {item.status}
                      </span>
                    </div>
        ...
        <section className="col-span-12 lg:col-span-4 space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-primary/10">
            ...
            <div className="space-y-4">
              <div className="bg-primary/5 p-4 rounded-xl border-l-4 border-primary">
                <p className="text-xs font-bold text-primary mb-1">집중 관찰 권고</p>
                <p className="text-sm font-bold mb-2 text-slate-900">박명수 어르신 (82세)</p>
                <p className="text-xs text-slate-600 leading-relaxed">
                  최근 48시간 내 심박수 변동폭이 평소 대비 25% 증가했습니다. 야간 불면 또는 급격한 컨디션 저하가 예상되니 <span className="font-bold text-primary">추가 혈압 측정 및 모니터링</span>을 제안합니다.
                </p>
              </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="bg-error-container text-error px-2 py-0.5 rounded text-[10px] font-bold">총 8명</div>
            </div>
            <div className="space-y-4">
              {[
                { name: '김철수 어르신', time: '오전 9시 (혈전용해제)', delay: '3시간 지연', status: 'delayed', photo: 'https://images.unsplash.com/photo-1552058544-f2b08422138a?auto=format&fit=crop&q=80&w=100&h=100' },
